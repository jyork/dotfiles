...
function host_type()
{
    ...
    case "${unameOut}" in
        ...
        Darwin*)    HOST_TYPE=Mac;;
        CYGWIN*)    HOST_TYPE=Cygwin;;
        MINGW*)     HOST_TYPE=MinGw;;
        *)          HOST_TYPE="UNKNOWN:${unameOut}"
    esac
}
host_type
...
function load()
{
    if [[ ${HOST_TYPE} == "Mac" ]]; then
        local load_total=`uptime | awk '{print $10}'`
        local cpu_count=`sysctl hw.ncpu | awk '{print $2}'`
        export SYSLOAD=`echo "100*$load_total/$cpu_count" | bc`
        echo $SYSLOAD
    else
        load SYSLOAD=$(cut -d " " -f1 /proc/loadavg | tr -d '.')
